...
def test_clean_proposal_passes(cfg):
    r = check(cfg, cfg.instruments[0], proposal(), equity=10000, book=FLAT,
              lots=0.13, events=[], now=NOW)
    assert r == []


def test_no_stop_loss_is_refused(cfg):
    r = check(cfg, cfg.instruments[0], proposal(sl=None), equity=10000, book=FLAT,
              lots=0.13, events=[], now=NOW)
...
def test_min_rr_recomputed_from_prices_not_from_the_models_claim(cfg):
    ...
    r = check(cfg, cfg.instruments[0], p, equity=10000, book=FLAT,
              lots=0.1, events=[], now=NOW)
...
def test_kill_switch_refuses_everything(cfg, tmp_path):
    ...
    (tmp_path / "KILL").touch()
    r = check(c, c.instruments[0], proposal(), equity=10000, book=FLAT,
              lots=0.13, events=[], now=NOW)
    assert "kill_switch" in gates_of(r)


def test_max_open_positions_counts_lab_tickets_only(cfg):
    book = Book(open_positions=3, open_lots=0.3, open_risk_currency=100,
                realized_today=0, floating=0)
    r = check(cfg, cfg.instruments[0], proposal(), equity=10000, book=book,
              lots=0.1, events=[], now=NOW)
...
def test_daily_loss_limit_halts_new_orders(cfg):
    ...
    book = Book(open_positions=1, open_lots=0.1, open_risk_currency=50,
                realized_today=-150.0, floating=-60.0)
    r = check(cfg, cfg.instruments[0], proposal(), equity=10000, book=book,
              lots=0.1, events=[], now=NOW)
...
def test_daily_loss_limit_not_breached_when_within(cfg):
    book = Book(open_positions=1, open_lots=0.1, open_risk_currency=50,
                realized_today=-50.0, floating=-20.0)
    r = check(cfg, cfg.instruments[0], proposal(), equity=10000, book=book,
              lots=0.1, events=[], now=NOW)
...
def test_high_impact_event_inside_window_blocks(cfg):
    ev = [{"event": "US CPI", "at": (NOW + timedelta(minutes=20)).isoformat(),
           "impact": "high", "affects": ["ICM:XAUUSD"]}]
    r = check(cfg, cfg.instruments[0], proposal(), equity=10000, book=FLAT,
              lots=0.13, events=ev, now=NOW)
    assert "news_blackout" in gates_of(r)


def test_event_outside_window_does_not_block(cfg):
    ev = [{"event": "US CPI", "at": (NOW + timedelta(minutes=90)).isoformat(),
           "impact": "high", "affects": ["ICM:XAUUSD"]}]
    r = check(cfg, cfg.instruments[0], proposal(), equity=10000, book=FLAT,
              lots=0.13, events=ev, now=NOW)
...
def test_event_for_another_symbol_does_not_block(cfg):
    ev = [{"event": "NZD rate decision", "at": (NOW + timedelta(minutes=10)).isoformat(),
           "impact": "high", "affects": ["NZDUSD"]}]
    r = check(cfg, cfg.instruments[0], proposal(), equity=10000, book=FLAT,
              lots=0.13, events=ev, now=NOW)
    assert "news_blackout" not in gates_of(r)
...
def test_low_impact_event_does_not_block(cfg):
    ev = [{"event": "minor survey", "at": NOW.isoformat(),
           "impact": "low", "affects": ["ICM:XAUUSD"]}]
    r = check(cfg, cfg.instruments[0], proposal(), equity=10000, book=FLAT,
              lots=0.13, events=ev, now=NOW)
    assert "news_blackout" not in gates_of(r)
