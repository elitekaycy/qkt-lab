...
CPI_EVENT = [{"event": "US CPI", "at": "2026-07-14T12:30:00Z", "impact": "high",
              "affects": ["ICM:XAUUSD"]}]
...
def test_candidate_beliefs_are_withheld(cfg, tmp_path):
    ...
    write(c.beliefs.path / "active-one.md",
          ["id: active-one", "status: ACTIVE", "statement: vwap pullbacks resolve up",
           "symbols: [ICM:XAUUSD]"], "n: 34 mean_R: +0.31")
    write(c.beliefs.path / "cand-one.md",
          ["id: cand-one", "status: CANDIDATE", "statement: fridays are bearish",
           "symbols: [ICM:XAUUSD]"], "n: 12")
...
def test_always_nodes_are_pinned(cfg, tmp_path):
    c = mem_cfg(cfg, tmp_path)
    write(tmp_path / "map" / "real-yields.md",
...
def test_budget_bounds_the_slice(cfg, tmp_path):
    ...
    c = mem_cfg(cfg, tmp_path)
    for i in range(50):
        write(tmp_path / "map" / f"node-{i:02}.md",
